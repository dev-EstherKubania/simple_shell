...
for dir in $directories; do
    ...
    if [ -n "$(find "$dir" -maxdepth 1 -type f \( -name '*.bash' ! -name 'runchecker.bash' \))" ]; then
        ...
        for script in "$dir"/*.bash; do
            ...
            if [ "$(basename "$script")" != "runchecker.bash" ]; then
                echo "./simple_shell_project/runchecker.bash ./hsh ./simple_shell_project/$(basename "$dir")/$(basename "$script")"
            fi
        done
    fi
